<?php
class ModelCheckoutUniorder extends Model {
    ...
    public function getOrder($order_id) {
        ...
        if ($order_query->num_rows) {
            ...
            /*  well   */
            $product_ids = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "'");
            /* well    */
    ...
    public function getOrders(int $limit = 0) {
        if($limit != 0) {
        $order_query = $this->db->query("SELECT *, (SELECT os.name FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = o.language_id) AS order_status FROM `" . DB_PREFIX . "order` o  ORDER BY o.date_added DESC LIMIT ".(int)$limit);
    } else {
            $order_query = $this->db->query("SELECT *, (SELECT os.name FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = o.language_id) AS order_status FROM `" . DB_PREFIX . "order` o  ORDER BY o.date_added DESC");

        }
        $product_ids = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_query->row['order_id'] . "'");
    ...
    public function getOrdersByDate(string $star_date, string $end_date) {
        if(isset($star_date) && isset($end_date)) {
            $star_date .= " 00:00:00";
            $end_date .= " 23:59:59";

            $order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` o  
            WHERE date_added  BETWEEN STR_TO_DATE('".$star_date."', '%Y-%m-%d %H:%i:%s') AND STR_TO_DATE('".$end_date."', '%Y-%m-%d %H:%i:%s') ORDER BY o.date_added DESC");
        } else {
            $order_query = $this->db->query("SELECT *, (SELECT os.name FROM `" . DB_PREFIX . "order_status` os WHERE os.order_status_id = o.order_status_id AND os.language_id = o.language_id) AS order_status FROM `" . DB_PREFIX . "order` o  ORDER BY o.date_added DESC");
